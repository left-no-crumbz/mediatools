...
def read_image_as_pil(
    ...
    if isinstance(image, Image.Image):
        ...
        try:
            img = Image.open(
                io.BytesIO(requests.get(image, stream=True).content)
                if str(image).startswith("http")
                else image
            )

            if img.mode in ("RGBA", "LA"):
                image_pil = img.convert("RGBA")
            else:
...
def pillow_to_base64(image: Image.Image) -> str:
    ...
    base64_src = f"data:image/{mime};base64,{image_str}"
    return base64_src
...
def image_comparison(
    ...
    # write html block
    htmlcode = f"""
		<style>body {{ margin: unset; }}
		.jx-image {{ background: transparent !important; }}
		.jx-slider {{ background: transparent !important; }}
		</style>
		{css_block}
		{js_block}
